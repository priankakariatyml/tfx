...
def _resolve_class_path(class_path: str) -> _ResolverOpClass:
  ...
  if not inspect.isclass(result) or not issubclass(
      result, (resolver_op.ResolverOp, resolver.ResolverStrategy)):
    raise TypeError(
        f'Invalid symbol {class_path}. Expected ResolverStrategy or ResolverOp '
        f'subclass but got {result}')
  return result
